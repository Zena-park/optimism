...
#### **6. 직접 배치 데이터 검증 (Direct Batch Data Verification)**
...
- [x] L2 배치 이벤트 모니터링 시스템 구현 (구현 완료)
  - [x] L1 컨트랙트 이벤트 구독 로직 (`MonitorL2BatchEvents()`)
  - [x] 배치 번호 추출 및 트리거 판단 (`extractBatchInfo()`)
  - [x] 자동 트리거 실행 (`processBatchTransaction()`)
  - [x] 배치 모니터링 시작/중지 기능 (`StartRATMonitoring()`, `StopL2BatchMonitoring()`)
  - [x] 기존 챌린저 L1 RPC 설정 재사용 (중복 제거 및 일관성 확보)
...
### **4. 직접 배치 데이터 검증 시스템** (`op-challenger/batch/verifier.go`)

#### **TODO 4.1: BatchVerifier 인터페이스 정의**
- [ ] `BatchVerifier` 인터페이스 정의
- [ ] L1 배치 데이터 추출 메서드 정의
- [ ] L2 배치 데이터 추출 메서드 정의
- [ ] 배치 데이터 비교 메서드 정의
- [ ] 확장 가능한 인터페이스 설계
...
#### **TODO 4.2: 기본 구현체 개발**
- [ ] `DefaultBatchVerifier` 구조체 구현
- [ ] L1 BatchInbox 컨트랙트 연동
- [ ] L2 RPC 클라이언트 연동
- [ ] 기본적인 배치 데이터 비교 로직 구현
- [ ] `BatchComparisonResult` 구조체 구현

#### **TODO 4.3: L1 배치 데이터 추출**
- [ ] L1 BatchInbox 컨트랙트에서 특정 블록 번호의 배치 데이터 조회
- [ ] 컨트랙트의 getBatch 함수를 호출하여 배치 데이터 추출
- [ ] 배치 데이터의 해시값과 원본 데이터를 함께 저장
- [ ] 에러 처리 및 재시도 로직

#### **TODO 4.4: L2 배치 데이터 추출**
...
#### **TODO 4.6: 단위 테스트**
- [ ] BatchVerifier 인터페이스 테스트
- [ ] L1 배치 데이터 추출 테스트
- [ ] L2 배치 데이터 추출 테스트
- [ ] 배치 데이터 비교 로직 테스트
- [ ] 에러 처리 테스트

### **5. EnhancedVerifier 통합** (`op-challenger/verification/enhanced.go`)

#### **TODO 5.1: 통합 검증기 구조**
- [ ] `EnhancedVerifier` 구조체 구현
- [ ] 기존 상태 루트 검증기와 새로운 배치 검증기 통합
- [ ] 두 검증 결과를 종합하여 최종 판단
- [ ] 로깅과 메트릭스를 통한 모니터링

#### **TODO 5.2: 검증 프로세스**
- [ ] 상태 루트 검증: 기존 방식대로 Dispute Game과 L2 실행 결과의 상태 루트 비교
- [ ] 배치 데이터 검증: 새로운 방식으로 L1과 L2 배치 데이터 직접 비교
- [ ] 통합 결과 결정: 두 검증 결과를 종합하여 최종 판단
...
#### **TODO 5.4: 서비스 통합**
- [ ] op-challenger 서비스 초기화 과정에 배치 검증기 추가
- [ ] 설정 파일에 배치 검증 관련 옵션 추가
- [ ] 기존 코드와의 호환성 보장
- [ ] 메트릭스 수집 시스템 통합
...
### **6. Reputation Manager** (`p2p/reputation/manager.go`)
...
### **7. Peer Attention Validator** (`p2p/attention/validator.go`)
...
### **8. 배치 검증 성능 최적화** (`op-challenger/batch/optimization.go`)
...
### **9. L2 Chain Recorder** (`p2p/l2/recorder.go`)
...
- [ ] 성능 목표 달성
- [ ] 보안 요구사항 충족
- [ ] 직접 배치 데이터 검증 시스템 정상 동작

### **품질 기준:**
- [ ] 테스트 커버리지 80% 이상
- [ ] 코드 리뷰 완료
- [ ] 문서화 완료
- [ ] 배치 검증 시스템 테스트 커버리지 90% 이상
...
- [optimism-security-model.md](../../../optimism-security-model.md) - Optimism 보안 모델
